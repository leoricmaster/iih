...
激活情报需求 × 已确认信源采集入口 → 内存 CollectionTask 列表，按各 IR 独立参数分发：
1. 生效窗口到期扫描：valid_until ≤ today 的激活 IR 触发自动关闭（落账 Close 提案）
2. 频率 due 过滤：未到采集节奏的 IR 跳过本轮（last_collected_at + 频率 > now）
3. 信源绑定过滤：IR 绑定信源 → 仅派单到这些信源的采集入口；空 → 全部已确认

IIH-06.01 通路反转：探索常驻——每个 due 需求无条件 +1 个探索任务（不绑入口，
...
class CollectionTask:
    """定向智能体任务化产物（内存对象，不落账）：单采集入口的常规采集任务。

    检索参数最简：无关键词过滤；URL 取自采集入口。
    """
...
class Director:
    ...
    def propose_tasks(self) -> tuple[list[CollectionTask], list[ExplorationTask]]:
        ...
        常规任务范围：已确认信源的全部采集入口（decision-05）。探索任务：每个 due 需求无条件一个。
        """
        ...
        for ir in due_irs:
            for entry in self._entries_for_ir(ir, entries):
                tasks.append(
                    CollectionTask(
    ...
    def _entries_for_ir(self, ir: IntelligenceRequirement, all_entries: list[Entry]) -> list[Entry]:
        """IR 的可用采集入口：绑定信源则仅取这些信源的入口；空则全部。
        ...
        return [e for e in all_entries if e.source_id in bound_source_ids]
