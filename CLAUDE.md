...
- 任务关联（文档章节、decision、原型）写入 References 字段（`task edit --add-ref`），描述末尾不另列关联行；Feature 与 US 的 type 设为 `feature`，技术/调研类用 `chore`/`spike`；判据：无新用户能力增量者（集成、交付形态改变、验收支撑）一律 `chore`，不套用户故事格式，故事句目的从句只写客户获得什么、验证手段入 DoD/plan。
