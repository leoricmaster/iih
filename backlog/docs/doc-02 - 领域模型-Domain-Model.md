...
反馈的后果分两层：**处置**（对被评价目标的即时动作：作废、重估、重评）与**学习**（三通路：信源信用、需求/采集配置、智能体迭代）。只有有效 / 事实错误动信用，经信用归因（decision-08）落到唯一责任信源；命题反馈仅作用于结论态，除事实错误触发重估外不改信用与配置；反馈默认可信（单人前提），多消费方出现时另行决策重启。
