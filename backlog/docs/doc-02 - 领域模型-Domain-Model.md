...
| 智能体 | 输入 | 输出（提案） |
|---|---|---|
| 定向 | 情报需求（订阅 / 任务）、探究任务（可含附件） | 采集任务：目标信源与媒介、检索参数、采集节奏；探究任务递归分解为子任务与情报需求（v1 手动 / v2 自动提案） |
...
**问题树走查**——「自动装车市场空间有多大」（疑问句，decision-15）：定向智能体按附件（测算框架）将 T2 分解为子任务（各测算法）+ 叶子情报需求；各子任务催生 IR → 采集 → 已核实情报入库 → 分析智能体判定出子命题；父任务分析智能体取子命题按交集定值框架合成父命题。底层情报作废 → 子命题变 → 父命题级联重估。
