...
| 媒介 / 载体 | 互联网 / 网页，记入溯源 |
| 情报条目 · 产出 | I1 = S1 的转载报道（产出自信源 S1，转引链穿透至 S2）；I2 = S2 的公告原文 |
| 命中（多对多） | I1 同时命中 R1 与 R2，I2 亦命中 R1 |
| 推理记录 · 证据 | 分析智能体判定 C1 证实（置信度：中），引用 I1 与 I2；C1 转为结论 |
...
反馈的后果分两层：**处置**（对被评价目标的即时动作：作废、重估、重评）与**学习**（三通路：信源信用、需求/采集配置、智能体迭代）。只有有效 / 事实错误动信用，经信用归因（decision-10）落到唯一责任信源；命题反馈仅作用于结论态，除事实错误触发重估外不改信用与配置；反馈默认可信（单人前提），多消费方出现时随 decision-06 一并重启。
