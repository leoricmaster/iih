...
一句话：**媒介是"在哪儿拿到的"，载体是"什么形态"，信源是"谁说的"，情报是"说了什么"，探究任务是"想知道什么"，命题是"信什么、求证什么"。**
...
| 智能体 | 输入 | 输出（提案） |
|---|---|---|
| 定向 | 情报需求（订阅 / 任务）、探究任务（可含方法学附件） | 采集任务：目标信源与媒介、检索参数、采集节奏；探究任务递归分解为子任务与情报需求（v1 手动 / v2 自动提案） |
| 采集 | 采集任务、原始素材（处理管线按载体：ASR / OCR / 解析） | 线索：陈述内容 + 溯源引用 |
| 审查 | 线索 | 候选情报，或否决为噪音（附理由） |
| 核实 | 候选情报，可检索外部信源佐证 | 评级（A1–F6）；无法完成则挂存疑，判定不实则否决 |
| 分析 | 已核实条目 / 子命题、命题及其候选 | 判定命题：证实 / 证伪 / 存疑，附置信度与推理记录；合成命题按框架取子命题合成 |
...
erDiagram
    ...
    消费方 ||--o{ 命题 : "登记（断言）"
    探究任务 ||--o{ 命题 : "答案落成 / 子结论"
    探究任务 ||--o{ 情报需求 : "催生（盯参数，decision-15）"
    命题 ||--o{ 情报需求 : "催生（盯证据，decision-15）"
    探究任务 ||--o{ 探究任务 : "分解为子任务（decision-15）"
...
| 探究任务 · 登记 | T1「W 与 Z 的合资传闻是否属实？（两周内查明）」 |
| 命题 · 登记 | 登记时转写 C1「W 与 Z 将成立合资公司」，先为假设（decision-15） |
| 探究任务 · 催生 | T1 催生任务型需求 R2「盯 W / Z 官方公告动向」，窗口至结题（decision-15） |
...
| 推理记录 · 证据 | 分析智能体判定候选答案 C1 证实（置信度：中），引用 I1；C1 转为结论 |
| 反馈 · 信用归因 | 用户对 I1 标「有效」→ 归因到链上最早陈述方 S2：S2 信用 ↑，如实转载的 S1 不动（decision-08） |

本例未走到：事实错误打在结论命题上 → 直接进入重估中（§6）。

**问题树走查**——「自动装车市场空间有多大」（疑问句，decision-15）：定向智能体按方法学附件将 T2 分解为子任务（各测算法）+ 叶子情报需求；各子任务催生 IR → 采集 → 已核实情报入库 → 分析智能体判定出子命题；父任务分析智能体取子命题按交集定值框架合成父命题。底层情报作废 → 子命题变 → 父命题级联重估。

| 实体 / 关系 | 本例 |
|---|---|
| 探究任务 · 登记 | T2「自动装车市场空间有多大？」 |
| 探究任务 · 分解 | T2 分解为子任务 T2a「单元存量法」、T2b「工作量产能法」… + 叶子 IR「特大型矿座数」等（decision-15） |
| 子探究任务 · 催生 | T2a 催生 IR「单元数 / 配置区间」，采集后已核实情报入库 |
...
要点：判定不实才否决——能完成评定的给弱评级（4–6）仍为已核实，核实无法完成才挂存疑；「噪音」限同源纯重复，跨信源同事件是印证，放行核实合并（decision-10）；已分发与已核实分离，以便度量需求覆盖。
...
要点：命题为一等对象，状态机锚点为命题本身（decision-15）；判定 = 出答案——叶子命题从情报、合成命题从子命题合成；每次状态迁移都落一条推理记录。结论是命题当前生效的判定，下游引用取当前值；已证伪默认稳态留底（负知识：避免重复提出、辅助排除）。
...
要点：探究任务只管问题生命周期（进行中→已结题），判定/置信度/推理记录归命题（状态机见 §4.2）；答案落成命题，可继续被引用与翻案，不受任务结题影响；探究任务可递归分解为子任务，结题即关催生的子需求与子任务，保留全部历史（decision-15）。
