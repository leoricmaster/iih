...
一句话：**媒介是"在哪儿拿到的"，载体是"什么形态"，信源是"谁说的"，情报是"说了什么"，课题是"想知道什么"，命题是"信什么、求证什么"。**
...
**拆解与合成，一降一升**（decision-15 的骨架）：课题侧自上而下——消费方登记课题，定向智能体按附件递归分解为子课题与专项需求（盯参数）；命题侧自下而上——叶子课题的答案从已核实条目判定（求证核实），非叶子课题的答案取子命题按框架合成（逻辑合成）。两线在每层「答案落成」处衔接：拆解产出的是**问题**，落成的才是**命题**；置信度沿命题合成 DAG 向根命题逐层计算，情报作废沿同一 DAG 触发级联重估。课题只管问题生命周期，命题独立存续、不受结题影响；独立登记的断言不挂课题，直接进命题侧求证。
...
| 智能体 | 输入 | 输出（提案） |
|---|---|---|
| 定向 | 情报需求（订阅 / 专项）、课题（可含附件） | 采集任务：目标信源与媒介、检索参数、采集节奏；课题递归分解为子课题与情报需求（v1 手动 / v2 自动提案） |
...
erDiagram
    ...
    消费方 ||--o{ 命题 : "登记（断言）"
    课题 ||--o{ 命题 : "答案落成 / 子结论"
    课题 ||--o{ 情报需求 : "催生（盯参数，decision-15）"
    命题 ||--o{ 情报需求 : "催生（盯证据，decision-15）"
    课题 ||--o{ 课题 : "分解为子课题（decision-15）"
...
| 课题 · 登记 | T1「W 与 Z 的合资传闻是否属实？（两周内查明）」 |
| 命题 · 登记 | 登记时转写 C1「W 与 Z 将成立合资公司」，先为假设（decision-15） |
| 课题 · 催生 | T1 催生专项需求 R2「盯 W / Z 官方公告动向」，窗口至结题（decision-15） |
...
| 实体 / 关系 | 本例 |
|---|---|
| 课题 · 登记 | T2「自动装车市场空间有多大？」 |
| 课题 · 分解 | T2 分解为子课题 T2a「单元存量法」、T2b「工作量产能法」… + 叶子 IR「特大型矿座数」等（decision-15） |
...
要点：课题只管问题生命周期（进行中→已结题），判定/置信度/推理记录归命题（状态机见 §4.2）；答案落成命题，可继续被引用与翻案，不受课题结题影响；课题可递归分解为子课题，结题即关催生的子需求与子课题，保留全部历史（decision-15）。子课题亦可单独结题（先行答出或放弃，如某测算法走不通），即关其催生的子需求，父课题按剩余子命题合成；结题后命题仍会被级联重估，需新证据时由消费方就该命题登记求证需求补采（两类催生见 §2）。
