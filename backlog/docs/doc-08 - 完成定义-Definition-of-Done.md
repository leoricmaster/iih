...
| 9 | 关闭任务时，若实现揭示长期文档（doc-01~doc-08）有更新必要，已更新沉淀——融入既有结构，不追加补丁句 | 全部 |

## 豁免规则

- 适用条件不满足的条目视为满足（如纯调研任务于 #2/#3/#4）。
- 确实做不到的条目不得勾选：先在任务 comment 留痕豁免项与理由，经用户批准后方可关闭。
- CI 质量门禁（ruff、mypy、format 检查、pytest-cov）由 IIH-01.01（承载工程骨架与 CI 基线工作包）建立，此后 #2/#3 持续生效。
