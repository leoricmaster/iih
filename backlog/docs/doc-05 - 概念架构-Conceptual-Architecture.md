...
| 反馈路由 | 类型化反馈分发到三通路、接收信用归因提案（decision-10） | 仅有效/事实错误动信用 |
...
**反馈与信用闭环**：消费方反馈 → 反馈路由按类型分流：有效/事实错误 → 信用归因提案（判断层判定责任信源）→ 信用计算器更新；事实错误（命题侧）→ 级联重估；不相关/过期 → 调度修正配置；重复/噪音 → 智能体迭代信号；评级异议 → 触发评级重评（decision-09）。信用更新是确定性计算，归因判定是判断层提案——分离保证可重放。
