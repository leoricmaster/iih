...
范围外：探究组页面（研究课题/命题/图谱）、推送通道、分发记录/警报、附件上传（IIH-02.01–03）、调度节奏配置。
...
author: Claude
created: 2026-09-15 09:09
---
MVP 全程浏览器验收通过（2026-09-15）。本单作为验收收尾 vehicle 关闭——六轮补救全部并入并已 push：第一轮 commit 70227ae（两跳采集+原文快照 MinIO+审查异议重审）、第二轮 commit 3fc9533（列宽记忆+折叠面板+紧凑表单+评级依据并一行+反馈区重构）、第三轮（工具栏布局+立即运行迁需求页+评级依据人话行）、第四轮 commit 2e94625（人工录入对齐文字纪要语义+内容指纹查重）、第五轮 commit 29f22c0（抽取提示词强化+条数强制 20+三处单行省略+录入页红 toast）、第六轮 commit 9cd30bb + 2e08e01（情报需求详情页采集概览只读区）。

CI 全绿（最近 commit 2e08e01 run 34950781216 success）。

留痕：DRAFT-09 需求级采集配置（来源/时效/频率三维度 m-0 全为常量、差异化待 m-1）+ DRAFT-10 「过期」反馈驱动采集配置学习通路，分别留后续。
---
<!-- COMMENTS:END -->
