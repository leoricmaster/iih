...
验收补救第三轮（2026-09-15）：信源库/情报需求页改工具栏布局（去 H1，表格上方一行：左页面名、右「＋ 登记」主按钮弹出原生 <dialog> 模态表单，ESC/点遮罩关闭、校验失败自动重开）、立即运行自顶栏迁需求页工具栏（跑的就是激活需求，语义归位；顶栏按钮移除）、评级依据改人话一行「独立信源 N · 信源档 R → 可信度 C」（公式串/版本号只在评级历史表留痕）、三处摘要行「互联网 / 网页」并作「互联网」（条目列表/详情/录入素材，元数据仍记全）、反馈理由标签「（补写，可选）」改「（可选）」。裁决留痕：「过期」反馈入口原样保留（路由去向配置通路，MVP 只记录）；同源条目评级字母可不同（字母=核实时点信源信用档快照，人工设档与反馈累计分并存所致，人工先验不进分数引擎——领域设计缝隙记后续块裁决，m-0 不动）。
